...
srv:listen(80,function(conn)
     conn:on("receive",function(sck,payload)
          ...
          if (string.find(paraStr,"=")~=nil) then
               fd = file.open("network_user_cfg.lua","r")
               print(fd)
               if(fd)then
               varCnt = fd:read()
               else
               ...
               fd = file.open("network_user_cfg.lua","w")
               print(varCnt)
               fdCnt = ""
               for name, value in string.gfind(paraStr, "([^&=]+)=([^&=]+)") do
                 if(varCnt) then
                      varCnt,matchCount =string.gsub(varCnt,decodeURI(name).."=\"[^\n]*[\n]",decodeURI(name).."=\""..decodeURI(value).."\"\n")
                      if(matchCount==0)then
                         varCnt = varCnt .. decodeURI(name).."=\""..decodeURI(value).."\"\n"
                      end
                 else
                    fdCnt = fdCnt .. decodeURI(name).."=\""..decodeURI(value).."\"\n"
                    varCnt = fdCnt
                 end
               end
               file.write(varCnt)
