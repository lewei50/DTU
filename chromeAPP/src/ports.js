	...
	if (err) {
	    ...
	    chrome.storage.sync.set({"svr": newSvrValue}, function() {
	      ...
	      console.log("setting svr to "+newSvrValue);
		    ...
		    chrome.storage.sync.set({"regCode": newValue}, function() {
		      console.log("setting regCode to "+newValue);
		    });
				if(newValue!="")
				{
			...
			{
				console.log('An error happened when open port'+sp);
				// if(sp!=null)
				// {
				// 	sp = null;
				// }
...
function connect(port, baudrate) {
	...
	strRcv = "";//received data string to resolve

	var output = document.getElementById("output");
	document.getElementById("settings").style.display = "none";

	var txtRcv = document.getElementById("txtRcv");

	sp.on("open", function() {
		//document.getElementById("sr_switch").src = "icon_switch_on.jpg";
		$('#sr_switch').attr('src',"icon_switch_on.jpg");
		$('#connectionStatus').text("Serial Connection Established");

		console.log("sp_connectionId:"+connectionId);
		connectLewei();
		// document.getElementById("connected-container").style.display = "block";
		// document.getElementById("feedbackSet").style.display = "none";
		//output.textContent += "Serial Connection Established\n";
		//
	});

	sp.on("error", function(string) {
		//output.textContent += "\nError: " + string + "\n";
		$('#connectionStatus').text("Error: " + string);
	});
    ...
    for(var i = 0; i < str.length; i++) {
     if(str.charCodeAt(i)<16)hexCharCode.push("0");
	...
	function send() {
		...
		//input.value = "";
		sp.writeString(line + "\n");
		//send to websocket server to verify and get feedback
		if(ws!= null)
		{
			if(strFeedBack != '')ws.send(strFeedBack+":"+line);
		}
	...
	function save()
	{
		...
		suggestedName: 'LOG.csv'},
		 ...
		 $("#saveResult").html("Save complete!");
		 };
		 writer.write(new Blob([document.getElementById("output").textContent],
		{type: 'text/plain'}));
		 }, function(e){
		 	$("#saveResult").html(e);
		});
...
SerialPort.prototype.onRead = function (readInfo) {
	if (readInfo && this.connectionId == readInfo.connectionId) {
		...
		this.publishEvent("dataString", readInfo.data);
	}
}
